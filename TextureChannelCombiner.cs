...
{
    [MenuItem("Window/brumTech/TextureChannelCombiner")]
    ...
    public void OnGUI()
    {
        ...
        GUILayout.BeginHorizontal();
        fillColor = GUILayout.HorizontalSlider(fillColor, 0f, 1f);
        if (GUILayout.Button(fillColor.ToString("#.#") + " Fill target texture's target channel"))
        {
            FillChannel();
            DebugTexture(target);
        }
